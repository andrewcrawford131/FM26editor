...
def generate_players_xml(
    ...
    for idx in range(count):
        ...
        fn = fixed_first_name if fixed_first_name is not None else name_rng.choice(first)
        sn = fixed_second_name if fixed_second_name is not None else name_rng.choice(sur)
        ...
        eff_gender_value = int(gender_value) if gender_value is not None else rng.randint(0, 1)
        eff_ethnicity_value = int(ethnicity_value) if ethnicity_value is not None else rng.randint(0, 10)
        eff_hair_colour_value = int(hair_colour_value) if hair_colour_value is not None else rng.randint(0, 6)
        eff_hair_length_value = int(hair_length_value) if hair_length_value is not None else rng.randint(0, 3)
        eff_skin_tone_value = int(skin_tone_value) if skin_tone_value is not None else rng.randint(0, 19)
        eff_body_type_value = int(body_type_value) if body_type_value is not None else rng.randint(1, 5)
        ...
        if parsed_second_nations:
            for _sn in parsed_second_nations:
            ...
            for _sn_idx, _sn in enumerate(parsed_second_nations, start=2):
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    ap.add_argument("--body_type_value", type=int, default=None, help="FM body type mapping (1..5)")
