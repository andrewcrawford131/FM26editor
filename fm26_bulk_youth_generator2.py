...
def _rep_triplet(rng: random.Random, rep_min: int, rep_max: int) -> Tuple[int, int, int]:
    """Return (current, home, world) with strict ordering: current > home > world."""
    rep_min = max(0, min(200, int(rep_min)))
    rep_max = max(0, min(200, int(rep_max)))
    if rep_max < rep_min:
        rep_min, rep_max = rep_max, rep_min
    if rep_max - rep_min >= 2:
        world = rng.randint(rep_min, rep_max - 2)
        home = rng.randint(world + 1, rep_max - 1)
        current = rng.randint(home + 1, rep_max)
        return current, home, world
    # Narrow range fallback
    current = rep_max
    home = max(rep_min, rep_max - 1)
    world = rep_min
    # if still not strict, allow ordered
    if not (current > home > world):
        vals = sorted([current, home, world], reverse=True)
        current, home, world = vals[0], vals[1], vals[2]
    return current, home, world
...
def generate_players_xml(
    ...
    fixed_right_foot: Optional[int] = None,    wage: int = 0,
    wage_min: int = 30,
    wage_max: int = 80,
    rep_current: int = -1,
    rep_home: int = -1,
    ...
    for idx in range(count):
        ...
        # wage (minimum 30)
        if wage > 0:
            wage_val = max(30, wage)
        else:
            wage_val = rng.randint(wage_min, wage_max)
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    ap.add_argument("--wage", type=int, default=0)  # fixed wage (>=30). If 0, uses wage_min..wage_max
