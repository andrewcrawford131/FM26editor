...
class App(tk.Tk):
    def __init__(self) -> None:
        ...
        self.fmdata_dir = self.fm_dir / "fmdata"
        try:
        ...
        ttk.Button(self.batch_details_actionbar, text="Show Output", command=self._toggle_output).pack(side="right")
        ...
        ttk.Button(self.single_details_actionbar, text="Show Output", command=self._toggle_output).pack(side="right")
        ...
        try:
            self._install_combobox_click_open_behavior()
        except Exception:
            pass
    ...
    def _add_details_section(self, parent, row: int, prefix: str) -> None:
        ...
        for r, (label, key, kind, values) in enumerate(rows):
            ...
            if kind == "combo":
                ...
                self._bind_searchable_combobox(cb, list(values or []))
    ...
    def _reload_master_library(self) -> None:
                ...
                for row in rdr:
                    ...
                    if kind == "club":
                        ...
                        if not dbid or not lg:
                            continue
                        label = name if name else f"Club DBID {dbid}"
                        clubs.append(label)
                        ...
                        if not dbid or not lg:
                            continue
                        label = name if name else f"City DBID {dbid}"
                        cities.append(label)
                        ...
                        if not dbid or not lg:
                            continue
                        label = name if name else f"Nation DBID {dbid}"
        ...
        for attr, values in [
            ...
            if cb is not None:
                try:
                    self._set_combo_values(cb, list(values))
                except Exception:
                    pass

        self._log(f"[OK] Loaded library pickers: clubs={len(clubs)}, cities={len(cities)}, nations={len(nations)}\n")

    def _install_combobox_click_open_behavior(self) -> None:
        """Allow comboboxes to open when clicking anywhere on the widget (not just the arrow)."""
        if getattr(self, "_combobox_click_open_bound", False):
            return

        def _open_on_click(event):
            try:
                w = event.widget
                if isinstance(w, ttk.Combobox) and str(w.cget("state")) != "disabled":
                    try:
                        ...
                        pass
                    # For editable/searchable combos, select all so typing replaces the default text.
                    if str(w.cget("state")) == "normal":
                        try:
                            if (w.get() or ""):
                                w.after_idle(lambda _w=w: (_w.selection_range(0, "end"), _w.icursor("end")))
                        except Exception:
                            pass
                    w.after(1, lambda _w=w: self._post_combobox_dropdown(_w))
            except Exception:
                pass
            return None

        try:
            self.bind_class("TCombobox", "<Button-1>", _open_on_click, add="+")
            self._combobox_click_open_bound = True
        except Exception:
            self._combobox_click_open_bound = False

    def _post_combobox_dropdown(self, combo: ttk.Combobox) -> None:
        """Open a ttk.Combobox dropdown programmatically (best-effort, cross-platform)."""
        try:
        ...
        try:
            combo.after_idle(_do_post)
        except Exception:
            _do_post()

    def _bind_searchable_combobox(self, combo: ttk.Combobox, values: list[str] | tuple[str, ...] | None = None) -> None:
        """Make a combobox filter values while typing (case-insensitive contains match)."""
        try:
            if values is not None:
                combo._all_values = list(values)  # type: ignore[attr-defined]
            elif not hasattr(combo, "_all_values"):
                combo._all_values = list(combo.cget("values") or [])  # type: ignore[attr-defined]
        except Exception:
            pass

        def _apply_filter(_event=None):
            try:
                all_vals = list(getattr(combo, "_all_values", list(combo.cget("values") or [])))
                q = (combo.get() or "").strip().lower()
                if q:
                    filtered = [v for v in all_vals if q in str(v).lower()]
                else:
                    filtered = all_vals
                combo.configure(values=filtered)
                # Dynamic search UX: show filtered dropdown immediately while typing.
                if str(combo.cget("state")) == "normal":
                    self._post_combobox_dropdown(combo)
            except Exception:
                pass

        def _reset_values(_event=None):
            try:
                combo.configure(values=list(getattr(combo, "_all_values", [])))
            except Exception:
                pass
            # Select all existing text so the next key press overwrites the default value.
            try:
                if str(combo.cget("state")) == "normal" and (combo.get() or ""):
                    combo.after_idle(lambda _c=combo: (_c.selection_range(0, "end"), _c.icursor("end")))
            except Exception:
                pass

        def _select_all_on_release(_event=None):
            try:
                if str(combo.cget("state")) == "normal" and (combo.get() or ""):
                    combo.after_idle(lambda _c=combo: (_c.selection_range(0, "end"), _c.icursor("end")))
            except Exception:
        ...
        def _accept_filtered_on_enter(_event=None):
            """Accept the current/highlighted filtered combobox result on Enter."""
            try:
                vals = list(combo.cget("values") or [])
                if not vals:
                    return None
                current_text = combo.get() or ""
                try:
                    idx = int(combo.current())
                except Exception:
                    idx = -1
                if 0 <= idx < len(vals):
                    chosen = str(vals[idx])
                elif any(str(v) == current_text for v in vals):
                    chosen = current_text
                else:
                    chosen = str(vals[0])
                combo.set(chosen)
        ...
        try:
            combo.bind("<KeyRelease>", _apply_filter, add="+")
            combo.bind("<FocusIn>", _reset_values, add="+")
            # Clicking into an editable combo with a default value should let typing replace it immediately.
            combo.bind("<ButtonRelease-1>", _select_all_on_release, add="+")
            # Press Enter to accept the current/highlighted filtered result immediately.
            combo.bind("<Return>", _accept_filtered_on_enter, add="+")
            combo.bind("<KP_Enter>", _accept_filtered_on_enter, add="+")
        except Exception:
            pass
    ...
    def _set_combo_values(self, combo: ttk.Combobox, values: list[str]) -> None:
        ...
        try:
            ...
            pass
        self._bind_searchable_combobox(combo, values)

    def _combo_state_for_mode(self, mode_var: tk.StringVar, combo: ttk.Combobox) -> None:
        try:
            combo.configure(state=("normal" if mode_var.get() == "fixed" else "disabled"))
        except Exception:
            pass
    ...
    def _build_batch_tab(self) -> None:
        ...
        self._bind_searchable_combobox(club_combo, [])
        ...
        self._bind_searchable_combobox(city_combo, [])
        ...
        self._bind_searchable_combobox(nation_combo, [])
...
# Development positions (2..19) - auto-selected by generator for multi-position profiles
    ...
    def _run_generator_common(
        ...
        # Safety restriction: players/people generated by this GUI must be age 100 or below.
        try:
            amin_i = int(str(age_min).strip())
            amax_i = int(str(age_max).strip())
        except Exception:
            messagebox.showerror("Age range", "Age min and Age max must be whole numbers.")
            return
        if amin_i < 1 or amax_i < 1 or amax_i < amin_i:
            messagebox.showerror("Age range", "Please enter a valid age range (min/max).")
            return
        if amin_i > 100 or amax_i > 100:
            messagebox.showerror("Age limit", "Maximum allowed age is 100.")
            return
    ...
    def _run_xml_appender(self) -> None:
        ...
        # Safety: if the appender script field was accidentally pointed at the extractor,
        # auto-correct to the XML appender in the FM script folder.
        try:
            if Path(script).name.lower() == "fm26_db_extractor.py":
                candidate = self.fm_dir / DEFAULT_XML_APPENDER_SCRIPT
                if candidate.exists():
                    self._log(f"[WARN] XML Appender script was set to extractor; auto-switching to:\n  {candidate}\n")
                    script = str(candidate)
                    self.appender_script.set(script)
        except Exception:
            pass
        ...
        try:
            if out_xml:
                ensure_parent_dir(out_xml)
            elif self.appender_create_target.get():
